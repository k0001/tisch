...
instance Tisch TTest where
  type UnRecHsR TTest = TestR
  type UnRecHsI TTest = TestW
  ...
  fromRecHsR' = \r -> return $ TestR
     (r ^. cola (C::C "c1"))
     (r ^. cola (C::C "c2"))
     (r ^. cola (C::C "c3"))
     (r ^. cola (C::C "c4"))
  toRecHsI' (TestW c1 c2 c3 c4) = rhiBuild $ \set_ -> HL.hBuild
     (set_ (C::C "c1") c1)
     (set_ (C::C "c2") c2)
     (set_ (C::C "c3") c3)
     (set_ (C::C "c4") c4)
...
query1 :: O.Query (RecPgR TTest, RecPgR TTest, RecPgR TTest, RecPgRN TTest)
query1 = proc () -> do
...
query2 :: O.Query (RecPgR TTest)
query2 = proc () -> do
  (t,_,_,_) <- query1 -< ()
  returnA -< t

outQuery2 :: Pg.Connection -> IO [RecHsR TTest]
outQuery2 conn = O.runQuery conn query2

query3 :: O.Query (RecPgRN TTest)
query3 = proc () -> do
  (_,_,_,t) <- query1 -< ()
  returnA -< t

outQuery3 :: Pg.Connection -> IO [Maybe (RecHsR TTest)]
outQuery3 conn = fmap mayRecHsR <$> O.runQuery conn query3

update1 :: Pg.Connection -> IO Int64
update1 conn = O.runUpdate conn tisch' upd fil
  where upd :: RecPgR TTest -> RecPgW TTest
        upd = over (cola (C::C "c3")) Just
            . over (cola (C::C "c4")) Just
        fil :: RecPgR TTest -> O.Column O.PGBool
        fil = \v -> eqc True (view (col (C::C "c1")) v)

outQuery1 :: Pg.Connection
          -> IO [(RecHsR TTest, RecHsR TTest, RecHsR TTest, Maybe (RecHsR TTest))]
outQuery1 conn = do
  xs :: [(RecHsR TTest, RecHsR TTest, RecHsR TTest, RecHsRN TTest)]
     <- O.runQuery conn query1
  return $ xs <&> \(a,b,c,d) -> (a,b,c, mayRecHsR d)
